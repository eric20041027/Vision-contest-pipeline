...
| 命令 | 作用 | 主要選項 |
|---|---|---|
| `vcp backup manifest` | 從結論反向走證據圖，寫 `configs/datasets/<name>/backup/<id>.json`（進 git、寫一次不改）：每個檔的角色、tier、sha、大小、服務的結論；checkpoint 以路徑為身分（每個路徑最新一筆），同名不同路徑的都會進清單 | `--dataset`、`--conclusion submission:<id>\|judgement:<prereg>\|run:<id>\|all`、`--id` |
