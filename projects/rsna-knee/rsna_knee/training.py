...
def training_context(name: str, plan: str, subset: str, config: Path, seed: int):
    ...
    if tuple(c.name for c in reader.dataset.card.categories) != NAMES:
...
def train(name: str, plan: str, subset: str, config: Path, out: Path, device: str):
    ...
    ids = [sid for sid in reader.ids if targets(reader.dataset.by_id[sid]) is not None]
    if not ids:
    ...
    labels = np.stack([targets(reader.dataset.by_id[sid]) for sid in ids])
        ...
        torch.save(
            {
                ...
                "plan": plan,
                "subset": subset,
                "samples_hash": reader.dataset.card.samples_hash,
