...
`--mode npy|png`、`--resize <長邊>`（只對 png）、`--stack-seq`（同 seq_id 的 view 依 seq_index 堆成 S×H×W；只對 npy 有意義）、`--window dicom|minmax|percentile`（png 的 8-bit 映射）、`--workers 1`、`--force`、`--decoder image|dicom`。
輸出 `datasets/<name>/cache/materialize/<mode>[-r<長邊>]/<sample dir>/<view>.<ext>` 與 `manifest.jsonl`（相對路徑，可整包搬到另一台機器）；失敗列在 `failed.jsonl`，`failed>0` → FAIL。重跑會跳過 resize / window / decoder / exif_policy 都相同且檔案仍在的輸出。
