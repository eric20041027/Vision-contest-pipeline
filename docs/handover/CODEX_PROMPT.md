...
- **發版**：一批工作合併後，若它改了寫進產物 / 台帳的內容或 CLI 契約就 bump MINOR，否則 PATCH（規則與四個步驟在 `CHANGELOG.md` 表頭；`__version__` 在 `src/vcp/__init__.py`，是唯一來源）。稽核 Wave 0 已於 2026-09-11 發為 `0.3.0`；Wave 1 起每個 wave 一個 MINOR。
