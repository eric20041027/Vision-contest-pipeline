...
`checkpoint` 條目：`train.yaml.uploads` 裡有 `sha256` 相符、`verified=true`、且 `name` 是這個路徑自己的遠端名稱（訓練層依路徑算出的名稱，或 0.10.0 以前的純檔名）的紀錄 → `kind=remote_copy`、`remote={dest, name}`（多筆取最新）；否則 `kind=file`。checkpoint 路徑經 `resolve_stored_path`，在 data root 外 → `root=external`。
