...
9. **RSNA 第二輪回報（2026-09-25）剩下的**：VCP-038 第 2–4 段（上傳前先讀平台、台帳位置可設定、多寫入者拓樸與 `merge=union`）、VCP-040 + 042（run 讀的證據檔與標籤集綁進紀錄）、VCP-041（dirty 工作樹 WARN 與 `--require-clean`，Wave 1c）都要先寫 spec；另有 0.10.0 審查記下的既有問題：同一 id 幾發分數不同時 `sync` 每次重寫一輪 `scored`（要一併重想 `latest_score`）、同 id 重傳的 `--force` 護欄（VCP-014）、平台列表 `ref` 的格式驗證。處置表在稽核文件第 16 節。
